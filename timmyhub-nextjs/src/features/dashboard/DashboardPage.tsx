...
export function DashboardPage() {
    ...
    const { activeTab, setActiveTab, openTabs, closeTab, handleAction } = useManagementTabs<Product>('Product');
    ...
    const columnDefs = useMemo<ColDef<Product>[]>(() => [
        ...
        { field: "price", headerName: "Price ($)", width: 110 },
        {
            ...
            cellRenderer: (params: ICellRendererParams) => (
                <Text c={params.value < 10 ? 'red' : 'green'} fw={700}>{params.value}</Text>
            )
        },
        {
            field: "status", headerName: "Status", width: 130,
            cellRenderer: (params: ICellRendererParams) => {
                const colors: Record<string, string> = { 'Active': 'green', 'Pending': 'yellow', 'Rejected': 'red' };
                return <Badge color={colors[params.value] || 'gray'} variant="light" mt={8}>{params.value}</Badge>;
            }
        },
        { field: "createdAt", headerName: "Created At", width: 140 },
        {
            headerName: "Actions", pinned: 'right', width: 150,
            cellRenderer: (params: ICellRendererParams<Product>) => (
                <Group gap={4} mt={4}>
                    <ActionIcon variant="subtle" color="cyan" size="sm" onClick={() => handleAction('Detail', params.data!)}><IconEye size={16} /></ActionIcon>
                    <ActionIcon variant="subtle" color="blue" size="sm" onClick={() => handleAction('Update', params.data!)}><IconEdit size={16} /></ActionIcon>
    ...
    const renderTabContent = (tab: TabItem<Product>) => (
        <Group p="xl">
            <Text>Content for {tab.label} ({tab.type})</Text>
        </Group>
    );
